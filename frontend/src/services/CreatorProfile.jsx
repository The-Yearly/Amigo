...
const Portfolio = () => {
  ...
  const flagUser = async () => {
    try {
      const res = await axios.post(
        ...
        {
          reportedUserId: creatorId, // Flag this profile's user
  ...
  if (!profile)
    return <div className="text-center mt-20 font-bold">User not found.</div>;
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans pb-20">
      ...
      <main className="max-w-6xl mx-auto px-4 mt-12 md:mt-16">
        {/* Profile Header */}
        <section className="flex flex-col md:flex-row gap-8 items-center md:items-start mb-12 relative">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex items-start justify-between">
              ...
              <button
                onClick={flagUser}
                className="flex items-center gap-2 px-4 py-2 text-red-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all duration-200 text-sm font-medium border border-red-200"
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="bg-[#053317] text-white p-8 rounded-2xl shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              <p className="text-xs opacity-70 mb-1 font-bold uppercase tracking-widest">
                Gigs Completed
              </p>
              <h2 className="text-5xl font-black">
                {profile.completedGigs || "24"}
              </h2>
            </div>
            <div className="absolute -right-4 -bottom-4 text-white/5 text-8xl font-black">
              ✓
            </div>
          </div>
          ...
          <div className="bg-pink-100 p-8 rounded-2xl shadow-sm flex items-center justify-between">
            <div>
              <h3 className="text-pink-900 font-black text-xl leading-tight">
                Campus
                <br />
                Verified
              </h3>
              <p className="text-pink-800 text-[10px] mt-2 font-bold uppercase">
                Trusted Student Creator
              </p>
            </div>
...
const PortfolioReviewCard = ({ name, text }) => (
  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
    <div className="flex items-center gap-3 mb-3">
      <div className="w-8 h-8 bg-pink-100 rounded-full flex items-center justify-center text-xs">
        👤
      </div>
      <div>
