...
export default function RequestListItem({
  ...
  if (status === "skeleton") {
    return (
      <div className="bg-surface-container-lowest p-6 rounded-xl shadow-ambient animate-pulse flex flex-col md:flex-row gap-6">
  ...
  return (
    <div
      onClick={onClick}
      className="group bg-surface-container-lowest p-6 rounded-xl shadow-ambient hover:scale-[1.01] transition-transform flex flex-col md:flex-row gap-6 relative overflow-hidden cursor-pointer"
    >
      {/* Pending accent stripe */}
      {isPending && <div className="absolute left-0 top-0 bottom-0 w-1 bg-secondary" />}
      ...
      <div className="flex-grow">
        ...
        <div className="flex justify-between items-start mb-2">
          <div>
            <span className={`${badgeClass} text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest mb-1 inline-block`}>
              {badgeLabel}
            </span>
            <h3 className="text-xl font-bold text-primary-container font-display">{title}</h3>
          </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-8 rounded-full bg-surface-container overflow-hidden shrink-0">
            <img src={requesterImg} alt={requesterName} className="w-full h-full object-cover" />
          </div>
          <div>
        ...
        {/* Excerpt */}
        <p className="text-sm text-on-surface-variant line-clamp-1 mb-4 italic">"{excerpt}"</p>
